...
Overall, the format one package definition has worked well.  Hundreds
of packages were converted from rosbuild to catkin using that
interface.  But, experience has revealed some areas that need
improvement.
...
Another lesser issue was the need to declare most ROS package
dependencies twice, using both ``<run_depend>`` and
``<build_depend>``.
...
characters and underscores and must start with a letter [2]_.  The
...
The ``<devel_depend>`` declares a transitive build dependencies.  A
 ...
 * ``<test_depend>`` can coexist with ``<build_depend>``.
...
Format one packages following REP-0127 [1]_ are not affected until
...
.. [9] Buildsystem mailing list discussion about different types of
   package dependencies
   (https://groups.google.com/forum/?fromgroups=#!topic/ros-sig-buildsystem/HKgOrdu1OO0)
